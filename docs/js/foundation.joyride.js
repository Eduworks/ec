    ...
    _createClass(Joyride, [{
      ...
      value: function _parseList() {
        ...
        this.$element.find('li').each(function (i) {
          var item = $.extend({}, {
            ...
            $visibleTarget: function(){return $($(this).data('target')+":visible")},
            isModal: function(){return !!!$($(this).data('target')).length},
      ...
      value: function _render(structure) {
        ...
        for (var s=0;s<structure.length;s++) {
          ...
          if ($(options.target+":visible").length) {
            // target element exists, create tooltip
            var tooltip = new Foundation.Tooltip($(structure[s].target+":visible"), {
            ...
            this.structure[s].isModal = false;
            $item = tooltip.template;

            // add joyride class to target
            $(structure[s].target+":visible").addClass(this.options.joyrideTargetClass);
          } else {
      ...
      value: function showItem(index) {
        var _this = this,
           ...
           * @event Joyride#show
           */
          this.structure[index].$visibleTarget().addClass(this.options.joyrideTargetActiveClass).trigger('show.zf.joyride');
          $('body').addClass(this.options.bodyActiveClass);
        };
        // scroll target into view if target exists
        if (this.structure[index].$visibleTarget().length) {
          var scrollTo = Math.max(0, this.structure[index].$visibleTarget().offset().top - this.options.scrollOffset),
      ...
      value: function hideItem(index) {
          if (this.structure[index].item !== undefined)
        if (this.structure[index].isModal) {
