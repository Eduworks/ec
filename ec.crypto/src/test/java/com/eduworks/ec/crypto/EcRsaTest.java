package com.eduworks.ec.crypto;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.stjs.javascript.functions.Callback1;
import org.stjs.testing.annotation.ScriptsBefore;
import org.stjs.testing.driver.STJSTestDriverRunner;
import window.library;

import static org.junit.Assert.assertTrue;
import static org.stjs.javascript.Global.console;

@RunWith(STJSTestDriverRunner.class)
@ScriptsBefore({"pem-jwk.js", "require.js", "/forge/forge.bundle.js", "ec.base.js"})
public class EcRsaTest {
	@Test
	public void rsaOaepTest() {
		console.log("-----rsaOaepTest-----");
		String randomString = EcAes.newIv(32);
		console.log("Generated string: " + randomString);
		EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		String encrypted = EcRsaOaep.encrypt(ppk.toPk(), randomString);
		console.log("Encrypted string: " + encrypted);
		String decrypted = EcRsaOaep.decrypt(ppk, encrypted);
		console.log("Decrypted string: " + decrypted);
		assertTrue(decrypted.equals(randomString));
	}

	@Test
	public void rsaUtf8OaepTest() {
		console.log("-----rsaUtf8OaepTest-----");
		String randomString = "ᚠᛇᚻ᛫ᛒᛦᚦ᛫ᚠᚱᚩᚠᚢᚱ᛫ᚠᛁᚱᚪ᛫ᚷᛖᚻᚹᛦᛚᚳᚢᛗ";
		console.log("Generated string: " + randomString);
		EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		String encrypted = EcRsaOaep.encrypt(ppk.toPk(), randomString);
		console.log("Encrypted string: " + encrypted);
		String decrypted = EcRsaOaep.decrypt(ppk, encrypted);
		console.log("Decrypted string: " + decrypted);
		assertTrue(decrypted.equals(randomString));
	}

	@Test
	public void rsaOaepAsyncNativeTest() {
		console.log("-----rsaOaepAsyncNativeTest-----");
		library.require("pem-jwk");
		final Callback1<String> log = new Callback1<String>() {
			@Override
			public void $invoke(String o) {
				console.log(o);
			}
		};
		String randomString = EcAes.newIv(32);
		console.log("Generated string: " + randomString);
		final EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		EcRsaOaepAsync.encrypt(ppk.toPk(), randomString, new Callback1<String>() {
			@Override
			public void $invoke(String encrypted) {
				console.log(encrypted);
				EcRsaOaepAsync.decrypt(ppk, encrypted, log, log);
			}
		}, log);
		assertTrue(true);
	}
	@Test
	public void rsaUtf8OaepAsyncNativeTest() {
		console.log("-----rsaUtf8OaepAsyncNativeTest-----");
		library.require("pem-jwk");
		final Callback1<String> log = new Callback1<String>() {
			@Override
			public void $invoke(String o) {
				console.log(o);
			}
		};
		String randomString = "ᚠᛇᚻ᛫ᛒᛦᚦ᛫ᚠᚱᚩᚠᚢᚱ᛫ᚠᛁᚱᚪ᛫ᚷᛖᚻᚹᛦᛚᚳᚢᛗ";
		console.log("Generated string: " + randomString);
		final EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		EcRsaOaepAsync.encrypt(ppk.toPk(), randomString, new Callback1<String>() {
			@Override
			public void $invoke(String encrypted) {
				console.log(encrypted);
				EcRsaOaepAsync.decrypt(ppk, encrypted, log, log);
			}
		}, log);
		assertTrue(true);
	}
	@Test
	public void rsaOaepAsyncNativeTestWorker() {
		console.log("-----rsaOaepAsyncNativeTestWorker-----");
		library.require("pem-jwk");
		final Callback1<String> log = new Callback1<String>() {
			@Override
			public void $invoke(String o) {
				console.log(o);
			}
		};
		String randomString = EcAes.newIv(32);
		console.log("Generated string: " + randomString);
		final EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		EcRsaOaepAsyncWorker.encrypt(ppk.toPk(), randomString, new Callback1<String>() {
			@Override
			public void $invoke(String encrypted) {
				console.log(encrypted);
				EcRsaOaepAsync.decrypt(ppk, encrypted, log, log);
			}
		}, log);
		assertTrue(true);
	}
	@Test
	public void rsaUtf8OaepAsyncNativeTestWorker() {
		console.log("-----rsaUtf8OaepAsyncNativeTestWorker-----");
		library.require("pem-jwk");
		final Callback1<String> log = new Callback1<String>() {
			@Override
			public void $invoke(String o) {
				console.log(o);
			}
		};
		String randomString = "ᚠᛇᚻ᛫ᛒᛦᚦ᛫ᚠᚱᚩᚠᚢᚱ᛫ᚠᛁᚱᚪ᛫ᚷᛖᚻᚹᛦᛚᚳᚢᛗ";
		console.log("Generated string: " + randomString);
		final EcPpk ppk = EcPpk.fromPem("-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----");
		EcRsaOaepAsyncWorker.encrypt(ppk.toPk(), randomString, new Callback1<String>() {
			@Override
			public void $invoke(String encrypted) {
				console.log(encrypted);
				EcRsaOaepAsync.decrypt(ppk, encrypted, log, log);
			}
		}, log);
		assertTrue(true);
	}
}
