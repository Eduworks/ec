...
public class CredentialCompetencyLocator {
    ...
    private void log(String s) {logBuffer.push(s);}
    ...
    private void registerTargetAsProcessed(String id) {targetsProcessed.$put(id,id);}
    ...
    private Array<ConditionProfile> getRequiredConditionProfilesList(Object requiresObj) {
        ...
        if (!EcArray.isArray(requiresObj)) {
            ...
            if (CONDITION_PROFILE_TYPE.equalsIgnoreCase(cw.getFullType())) cpa.push(generateConditionProfile(requiresObj));
        }
        else {
            ...
            for (int i=0;i<oa.$length();i++) {
                ...
                if (CONDITION_PROFILE_TYPE.equalsIgnoreCase(cw.getFullType())) cpa.push(generateConditionProfile(oa.$get(i)));
            }
        }
        return cpa;
    }
