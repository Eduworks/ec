...
public class RollupRulePacketGenerator 
{
   ...
   public RollupRulePacketGenerator(InquiryPacket ip) 
   {
      this.ip = ip;
   ...
   private boolean hasOrOperation() 
   {
      ...
      {
         if (OperationType.OR.equals(queryOperations.$get(i))) return true;
      }
      return false;
   }
   ...
   {
      if (hasOrOperation()) return IPType.COMBINATOR_OR;
      return IPType.COMBINATOR_AND;
   }
   
   private InquiryPacket generateComboAndPacket() 
   {
      return new InquiryPacket(ip.subject,null,null,ip.context,null,ip.failure,null,IPType.COMBINATOR_AND);
   }
   
   private InquiryPacket generateRollupRulePacket(String rule) 
   {
      return new InquiryPacket(ip.subject,null,null,ip.context,null,ip.failure,rule,IPType.ROLLUPRULE);
   }
   ...
   {
      if (queryOperations.$length() <= 0) return;
      InquiryPacket currentAndPacket = generateComboAndPacket();
      OperationType priorOt;
      if (OperationType.OR.equals(queryOperations.$get(0))) rollupIp.subPackets.push(generateRollupRulePacket(queries.$get(0)));
      else currentAndPacket.subPackets.push(generateRollupRulePacket(queries.$get(0)));
      priorOt = queryOperations.$get(0);
      for (int i=1;i<queryOperations.$length();i++) 
      {         
         if (OperationType.OR.equals(queryOperations.$get(i)))
         {
            if (OperationType.OR.equals(priorOt)) rollupIp.subPackets.push(generateRollupRulePacket(queries.$get(i)));
            else
            { 
         ...
         else 
         {
            ...
            else currentAndPacket.subPackets.push(generateRollupRulePacket(queries.$get(i)));             
         }  
         priorOt = queryOperations.$get(i);
      }
      if (OperationType.OR.equals(priorOt)) rollupIp.subPackets.push(generateRollupRulePacket(queries.$get(queries.$length() - 1)));
      else 
      {
   ...
   public InquiryPacket generatePacket() 
   {
      ...
      InquiryPacket rollupIp = new InquiryPacket(ip.subject,null,null,ip.context,null,ip.failure,null,ipt);
      if (IPType.COMBINATOR_AND.equals(ipt)) addAllQueries(rollupIp);
      else buildQueryTree(rollupIp);
      return rollupIp;      
   }   
   
   public static void main (String[] args) throws Exception 
   {
//      InquiryPacket ip = new InquiryPacket(null,null,null,null,null,null,null,null);
...
//         if (sp.type.equals(IPType.ROLLUPRULE)) System.out.println("   " + sp.type + " - " + sp.rule);
//         else 
//         {
//            System.out.println("   " + sp.type);
//            for (int j=0;j<sp.subPackets.$length();j++)
//            { 
//               System.out.println("      " + sp.subPackets.$get(j).type + " - " + sp.subPackets.$get(j).rule);
//            }
//         }
//      }
