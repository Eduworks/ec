...
public class RollupRuleInterfaceTest
{
	...
	{
		String input = 
		      "[competency:http://skyrepo.eduworks.com/service/data/schema.eduworks.com.cass.0.1.competency/d885dcd8-f00b-4ccf-82d8-ee14d6c84ef0 AND confidence>0.6]" +
...
//		String input = "[competency:http://Addition AND confidence>0.6] AND " + 
//            "[competency:http://Subtraction AND confidence>0.6] AND " +
//            "[(competency:http://Algebra OR competency:http://Calculus) AND confidence>0.0] AND " +
//            "[competency:http://Multiplication AND confidence>0.6] OR " + 
//            "[competency:http://Division AND confidence>0.6]";
		...
		rri.success = new Callback1<Boolean>()
	      {
	         ...
	         {
	            if (ip.subPackets == null || ip.subPackets.$length() <= 0) Global.console.log("NO SUB PACKET GENERATED!!!");
	            else 
	            {	               
                  ...
                  {  
                     ...
                     if (sp.type.equals(IPType.ROLLUPRULE)) Global.console.log("    " + sp.type + " - " + sp.rule);
                     else 
                     {
